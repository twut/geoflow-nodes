...
class KdTree {
public:
    //think about a struct to save all the information below
    //level of kdtree
    int levelcount = 0;
    std::vector<Vector3D> m_maxpoint;
    std::vector<Vector3D> m_minpoint;
    ...
    struct sphere {
        ...
        {
            return(s1.pos == pos && s1.radius == radius&&s1.index ==index);
        }
    };
    ...
    //std::vector<std::vector <KdTreePoint*>> m_boxKdTreePoint;

    //std::vector<std::vector <int>> m_boxKdTreePoint;
    //const Vector3D *m_treePoints;
    ...
    Vector3D centerofBoundingBox() {
        Vector3D center;
        center.x = (m_boundingBoxHighCorner.x + m_boundingBoxLowCorner.x)*0.5;
        center.y = (m_boundingBoxHighCorner.y + m_boundingBoxLowCorner.y)*0.5;
        center.z = (m_boundingBoxHighCorner.z + m_boundingBoxLowCorner.z)*0.5;
        return center;
    }
