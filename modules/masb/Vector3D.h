...
public:
	...
	// constructors
	/////////////////////////////////////////////////////////////////	
	inline Vector3D (void)restrict(amp, cpu) {};
	...
	inline float& operator[] (int Index) //restrict(cpu,amp)
	{
		return (p[Index]);
	};
	...
	/*inline Vector3D& operator= (const Vector3D& P)restrict(amp,cpu)
	{
		p[0] = P[0];
		p[1] = P[1];
		p[2] = P[2];
	...
	inline Vector3D operator- (const Vector3D P) const //restrict(amp,cpu)
	{
		Vector3D res;
		res[0] = p[0] - P[0];
		res[1] = p[1] - P[1];
