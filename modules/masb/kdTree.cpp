...
KdTree::KdTree(const Vector3D *positions, unsigned int nOfPositions, unsigned int maxBucketSize) {
	...
	for (unsigned int i = 0; i < nOfPositions; i++) {
		m_points[i].pos.x = positions[i].x;
        m_points[i].pos.y = positions[i].y;
        m_points[i].pos.z = positions[i].z;
...
void KdTree::createTree(KdNode &node, int start, int end, Vector3D maximum, Vector3D minimum) {
    ...
    //std::cout << "Test depth of tree:" << levelcount << std::endl;
	int	mid;
	...
	if (mid - start <= m_bucketSize) {
        ...
        ///////////////////////////////////
       /* KdTreePoint* a = m_points;
        std::vector<int> vecKdPoints;
        for (int s = 0; s < start; s++) {
                      
            vecKdPoints.push_back(a->index);
            a= a + 1;
        }*/
        /////////////////////////////////
        ...
        //m_boxKdTreePoint.push_back(vecKdPoints);
	...
	if (end - mid <= m_bucketSize) {
        ...
        ////////////////////
        /*KdTreePoint* b = m_points;
        std::vector<int> v2;
        for (int k = 0; k < mid; k++) {
            
            v2.push_back(b->index);
            b = b + 1;
