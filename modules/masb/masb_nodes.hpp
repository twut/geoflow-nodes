...
namespace geoflow::nodes::mat {
  ...
  class NegNormalDetector : public Node 
  {
  public:
      using Node::Node;
      void init()
      {
          ...
          add_output("pc", typeid(PointCollection));
          add_output("normals_fixed", typeid(vec3f));
      }
      void process();
  ...
  public:
      ...
      static float if_interiorMAT(Vector3D pc, Vector3D interior_MAT)
      {                 
          Vector3D point_above(0, 0, 1000);
          Vector3D mat_pt = interior_MAT - pc;
          float a = dotProduct(point_above, mat_pt);
          return a;
